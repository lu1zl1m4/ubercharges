...
app = Flask(__name__)
app.config["SECRET_KEY"] = "TT-jNKTKX7CJ7hpX5fbY9WtrjKk"
...
def login():
    ...
    adm = testeadm_1110.Adm(senha, bd, cpf)
    ...
    if request.method == 'POST':
        if request.form['button'] == 'Login':
            ...
            if adm.verify_login(cpf, senha) == 1:
                Id_adm = adm.retorna_id_adm_logado(cpf, senha)
                session ['Id_adm'] = Id_adm 
                return render_template('PaginaPrincipal.html')

            elif adm.verify_login(cpf, senha) == 2:
                return render_template('SenhaIncorreta.html') 

            elif adm.verify_login(cpf, senha) == 3:
                return render_template('CPFAdmIncorreto.html')                
 
            elif adm.verify_login(cpf, senha) == 4: 
...
def redefinir_senha():
    ...
    nova_senha = str(request.form["NovaSenha"])
    
    bd = bancodedados.BancoDados
    adm = testeadm_1110.Adm(Nome, nova_senha, bd, cpf)

    if request.form['button'] == 'Seguinte':
        if adm.verify_user(Nome, cpf):
            id_user = adm.retorna_id_user_para_redefinir_senha(Nome, cpf)
            bd.update_value(id_user, Nome, nova_senha, cpf)
            return render_template('SenhaAlterada.html')
        
        if not adm.verify_user(Nome, cpf):
            return render_template('ADMNaoCadastrado.html')

    if request.form['button'] == 'Voltar':
        return render_template('LoginADM.html')

@app.route("/SenhaRedefinidaADM", methods=["POST"])
def voltar2():
    if request.form['button'] == 'Voltar':
        return render_template('LoginADM.html')
...
# LOGO, ESSA PARTE ESTÁ EM ANÁLISE.     
    ...
    bd.criar_tabela_motorista()
    bd.criar_tabela_motorista_vs_adm()
    
    if request.method == 'POST':
        
        if request.form['button'] == 'Cadastrar':
            if not mot.verify_motorista(nome,documento):
                bd.insert_val_motorista(nome, documento)
                Id_Mot = mot.retorna_id_mot_cadastrado()
                bd.insert_ids_mot_vs_adm(session ['Id_adm'], Id_Mot)
...
@app.route("/MotoristaCadastrado", methods=["POST"])
def PaginaPrincipal():
